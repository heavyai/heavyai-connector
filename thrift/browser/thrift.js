...
var Thrift = {
   ...
   */
  inherits(constructor, superConstructor, name) {
    function F() {}
...
Thrift.TXHRTransport.prototype = {
  ...
  flush(async, callback) {
    ...
    xreq.open("POST", this.url, Boolean(async))

    // add custom headers
    Object.keys(self.customHeaders).forEach((prop) => {
   ...
   */
  jqRequest(client, postData, args, recv_method) {
    if (
...
Thrift.Protocol.prototype = {
  ...
  writeString(str) {
    ...
    if (str === null) {
      ...
      for (let i = 0; i < str.length; i++) {
        let ch = str.charAt(i) // a single double quote: "
        if (ch === '"') {
          escapedString = escapedString + '\\"' // write out as: \"
        } else if (ch === "\\") {
          // a single backslash
          escapedString = escapedString + "\\\\" // write out as double backslash
        } else if (ch === "\b") {
          // a single backspace: invisible
          escapedString = escapedString + "\\b" // write out as: \b"
        } else if (ch === "\f") {
          // a single formfeed: invisible
          escapedString = escapedString + "\\f" // write out as: \f"
        } else if (ch === "\n") {
          // a single newline: invisible
          escapedString = escapedString + "\\n" // write out as: \n"
        } else if (ch === "\r") {
          // a single return: invisible
          escapedString = escapedString + "\\r" // write out as: \r"
        } else if (ch === "\t") {
          // a single tab: invisible
          escapedString = escapedString + "\\t" // write out as: \t"
        } else {
          escapedString = escapedString + ch // Else it need not be escaped
        }
      }
  ...
  readMessageBegin() {
    ...
    const message = this.transport.readAll()

    console.log("t message", message)

    if (typeof JSON !== "undefined" && typeof JSON.parse === "function") {
      this.robj = JSON.parse(message)
    } else if (typeof jQuery !== "undefined") {
      this.robj = jQuery.parseJSON(message)
    } else {
      this.robj = eval(message)
    }
  ...
  readFieldBegin() {
    ...
    // get a fieldId
    for (let f in this.rstack[this.rstack.length - 1]) {
      if (f === null) {
        continue
      }
    ...
    if (fid != -1) {
      ...
      // way to match a key
      for (let i in this.rstack[this.rstack.length - 1]) {
...
Thrift.Multiplexer.prototype.createClient = function(
  ...
  const client = new SCl(new Thrift.MultiplexProtocol(serviceName, transport))

  return client
}

let copyList, copyMap
